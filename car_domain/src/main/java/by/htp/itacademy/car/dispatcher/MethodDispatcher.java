package by.htp.itacademy.car.dispatcher;
...
import by.htp.itacademy.car.annotation.util.ServletContainer;
import by.htp.itacademy.car.chaincasttype.FacadeCast;
			...
			if (map.containsKey(String.valueOf(arg))) {
				if (ReqParam.class.getTypeName().equals(map.get(arg).annotationType().getTypeName())) {
					...
					if (requestParameterValue != null) {
						methodParameters[j] = FacadeCast.getCastChain().getValue(params[j].getType(), requestParameterValue);
					} else {
						methodParameters[j] = FacadeCast.getCastChain().getValue(params[j].getType(), reqParam.defaultValue());
					}
					...
					Gson gson = new Gson();
					String jsonString = gson.toJson(new User());//getJsonString(request);
